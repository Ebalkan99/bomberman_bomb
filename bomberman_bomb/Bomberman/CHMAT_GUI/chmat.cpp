...
int CHMAT::set(int x, int y, CHMAT& r)
{
    if (x<1 || x>X) return (-1);
    (data)[(y - 1) * X + (x - 1)] = r(x,y);
    return 0;
}
...
void Line(CHMAT& target,CHMAT& r, int x1, int y1, int x2, int y2, int color) {
...
void Line_for_ellipse(CHMAT& target, CHMAT& r, int x1, int y1, int x2, int y2, int color, bool isFill, bool delet, bool leftDelet, bool rightDelet, int o_x, int o_y, int width, int height) {//ismet
    int i, dx, dy, sdx, sdy, dxabs, dyabs, x, y, px, py;
    ...
    if (dxabs >= dyabs) /* the line is more horizontal than vertical */
    {
        for (i = 0; i < dxabs; i++)
        {
            ...
            px += sdx;
            if (isFill && delet && leftDelet) {
                for (int i = 0; i < o_x - x1; i++)
                {
                    target.set(px + i, py, r);
                    target.set(o_x + o_x - (px + i), py, r);
                    target.set(px + i, (o_y - (py - o_y)), r);
                    target.set(o_x + o_x - (px + i), (o_y - (py - o_y)), r);

                }
            }
            else if (isFill && delet && rightDelet) {
                for (int i = 0; i < o_x - x1; i++)
                {
                    target.set(px + i, py, r);
                    target.set(o_x + o_x - (px + i), py, r);
...
bool Ellipse(CHMAT& target,CHMAT& r, int x, int y, int width, int height, int col, bool isFill, bool delet, bool leftDelet, bool rightDelet)
{
    ...
    for (int i = 0; xlen >= counter1 || ylen >= counter2; i++)
    {
        ...
        if (target.set(holder.x, holder.y, r) == 0) result = true;
        Line(target,r, holder.x, holder.y, pre_point_x, pre_point_y, col);

        if (target.set(holder.x + 2 * (O.x - holder.x), holder.y, r) == 0) result = true;
        Line(target,r, holder.x + 2 * (O.x - holder.x), holder.y, pre_point_x + 2 * (O.x - pre_point_x), pre_point_y, col);


        if (target.set(holder.x, holder.y - 2 * (holder.y - O.y), r) == 0) result = true;
        Line(target,r, holder.x, holder.y - 2 * (holder.y - O.y), pre_point_x, pre_point_y - 2 * (pre_point_y - O.y), col);

        if (target.set(holder.x + 2 * (O.x - holder.x), holder.y - 2 * (holder.y - O.y), r) == 0) result = true;
        Line(target,r, holder.x + 2 * (O.x - holder.x), holder.y - 2 * (holder.y - O.y), pre_point_x + 2 * (O.x - pre_point_x), pre_point_y - 2 * (pre_point_y - O.y), col);


        if (isFill && delet && leftDelet) {
            if (pre_point_y != holder.y)Line_for_ellipse(target,r, pre_point_x, pre_point_y, holder.x, holder.y, col, true, true, true, false, O.x, O.y, width, height);

            for (int j = 0; j < O.x - holder.x; j++)
            {
                target.set(holder.x + j, holder.y, r);

                target.set(O.x + j, holder.y, r);

                target.set(holder.x + j, holder.y - 2 * (holder.y - O.y), r);
                target.set(O.x + j, holder.y - 2 * (holder.y - O.y), r);

            }
        }
        else if (isFill && delet && rightDelet) {
            if (pre_point_y != holder.y)Line_for_ellipse(target, r, pre_point_x, pre_point_y, holder.x, holder.y, col, true, true, false, true, O.x, O.y, width, height);
            ...
            if (pre_point_y != holder.y)Line_for_ellipse(target,r, pre_point_x, pre_point_y, holder.x, holder.y, col, true, false, false, false, O.x, O.y, width, height);
